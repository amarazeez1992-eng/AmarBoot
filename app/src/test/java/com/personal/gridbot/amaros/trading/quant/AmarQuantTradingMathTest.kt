...
class AmarQuantTradingMathTest {
    ...
    @Test fun atrUsesWilderSmoothingInsteadOfSimpleRollingAverage() {
        assertEquals(2.25, AmarQuantTradingMath.atr(listOf(1.0, 2.0, 3.0), 2))
        assertEquals(2.0, AmarQuantTradingMath.atr(listOf(1.0, 2.0, 3.0, 2.0), 2))
    ...
    @Test fun volatilityDrawdownAndVarUseExplicitStatisticalDefinitions() {
        ...
        assertEquals(3.85, AmarQuantTradingMath.historicalVar(listOf(1.0, 2.0, 3.0, 4.0), 0.95))
    ...
    @Test fun riskOfRuinUsesExplicitDrawdownBarrierAndModelAssumptions() {
        ...
        assertEquals(1.0, nonPositiveLogGrowth)
