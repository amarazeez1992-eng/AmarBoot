...
class AdmissionContractsTest {
    ...
    fun admitted_finding_rejects_invalid_relevance_scores() {
        assertFailsWith<IllegalArgumentException> {
            AdmittedFinding(finding(), AdmissionState.REJECTED, -0.01, "test")
        }
        assertFailsWith<IllegalArgumentException> {
            AdmittedFinding(finding("https://example.test/one"), AdmissionState.REJECTED, 1.01, "test")
        }
        assertFailsWith<IllegalArgumentException> {
            AdmittedFinding(finding("https://example.test/two"), AdmissionState.REJECTED, Double.NaN, "test")
        }
        assertFailsWith<IllegalArgumentException> {
            AdmittedFinding(finding("https://example.test/three"), AdmissionState.REJECTED, Double.POSITIVE_INFINITY, "test")
        }
    }
    ...
    fun admission_result_enforces_state_invariants() {
        ...
        val rejected = AdmittedFinding(
            finding("https://example.test/rejected"),
    ...
    fun admission_result_rejects_wrong_state_in_either_partition() {
        ...
        val admittedState = AdmittedFinding(
            finding("https://example.test/admitted"),
