...
class AmarStageFourBacktestTest {
    ...
    fun deterministic_backtest_is_reproducible_and_reports_metrics() {
        ...
        assertEquals(0.5, first.trades[0].netPnl, 0.000001)
        assertEquals(3.5, first.trades[1].netPnl, 0.000001)
        assertEquals(2.5, first.trades[2].netPnl, 0.000001)
        assertEquals(1.5, first.trades[3].netPnl, 0.000001)
        assertEquals(8.0, first.trades.sumOf { it.netPnl }, 0.000001)
        assertEquals(1008.0, first.finalEquity, 0.000001)
