...
class AmarVerificationLayerTest {
    private fun finding(title: String, uri: String, evidence: String, stance: EvidenceStance, retrievedAt: Long = 1_000L) =
        ResearchFinding(title, uri, evidence, Authority.OFFICIAL, stance, retrievedAtEpochMs = retrievedAt)
    ...
    fun verifies_clean_evidence_and_builds_provenance_chain() {
        ...
        val report = AmarVerificationLayer().verify("The policy requires verification before release.", findings, 2_000L)
    ...
    @Test
    fun evidence_only_verification_does_not_fabricate_a_claim() {
        val findings = listOf(
        ...
        val report = AmarVerificationLayer().verifyEvidenceOnly(findings, 2_000L)
        assertEquals(AmarVerificationStatus.VERIFIED, report.status)
    ...
    @Test
    fun opposing_overlapping_evidence_is_detected_and_prevents_verified_status() {
        val findings = listOf(
            finding("Support", "https://a.example/support", "The change is safe and approved for release.", EvidenceStance.SUPPORTS),
            finding("Oppose", "https://b.example/oppose", "The change is unsafe and should not be approved for release.", EvidenceStance.OPPOSES)
        )
        val report = AmarVerificationLayer().verify("The change is safe and approved for release.", findings, 2_000L)
        assertEquals(1, report.conflicts.size)
        assertNotEquals(AmarVerificationStatus.VERIFIED, report.status)
    }

    @Test
    fun unrelated_opposing_evidence_is_not_a_false_positive_conflict() {
        val findings = listOf(
            finding("Support", "https://a.example/support", "The release gate is complete.", EvidenceStance.SUPPORTS),
            finding("Oppose", "https://b.example/oppose", "The weather forecast predicts rain tomorrow.", EvidenceStance.OPPOSES)
        )
        val report = AmarVerificationLayer().verifyEvidenceOnly(findings, 2_000L)
    ...
    fun malformed_source_or_empty_evidence_cannot_be_verified() {
        val report = AmarVerificationLayer().verify("A claim that cannot be verified from evidence.", listOf(
    ...
    fun provenance_is_deterministic_for_identical_inputs() {
        ...
        assertEquals(AmarProvenanceChain.build(findings), AmarProvenanceChain.build(findings))
    }
}
