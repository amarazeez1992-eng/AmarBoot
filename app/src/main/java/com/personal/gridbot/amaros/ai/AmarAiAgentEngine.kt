...
/** AI Supervisor boundary: advisory, evidence-driven and approval-gated. */
class AmarAiAgentEngine(
    ...
    private fun buildPrompt(request: String): String {
        ...
        val contextJson = JSONObject().put("market", JSONObject()
            ...
            .put("capabilities", "market analysis, strategy design, strategy critique, strategy notebook, Bot Lab, B21 testing, B22 deterministic simulation, chart analysis, external research, voice/image input")
...
Challenge incomplete strategies, explain why, propose improvements, compare alternatives, and request user approval before changing an approved strategy or authorizing broker execution.
...
You may inspect the app, test deterministic strategy logic, search registered/local evidence and save/load strategy notes through tools.
External research must be clearly marked as external and unverified until independently validated.
Broker execution is NOT available in the current build. Never claim that an order was sent, opened, closed or modified.
When future MT5 transport is connected, execution must still pass through command authority, emergency stop, validation, idempotency and post-execution verification.
...
""".trimIndent()
    ...
    private fun executeTool(tool: String, args: String): String? {
        return when (tool) {
            "library_search" -> {
            ...
            "strategy_save" -> {
                ...
                if (repo == null) {
                    "strategy_save => unavailable: AI engine has no Android context"
                } else {
            ...
            "strategy_load" -> {
                val repo = context?.let { AmarAiStrategyNotesRepository(it) }
                if (repo == null) "strategy_load => unavailable: AI engine has no Android context"
                else {
