...
class AmarAgentPlanner {
    fun plan(request: AmarAgentRequest, availableTools: List<AmarAgentTool>): AmarAgentPlan {
        val intent = classify(request.userText)
        ...
        return AmarAgentPlan(
            ...
            steps = listOf(
                ...
                if (intent == AgentIntent.TRADE_ANALYSIS) "simulate_and_risk_check" else "produce_answer",
                "audit_output"
            ),
            requiredTools = tools,
            stopConditions = listOf("missing_evidence", "policy_denied", "unsafe_execution_request")
        )
    }
