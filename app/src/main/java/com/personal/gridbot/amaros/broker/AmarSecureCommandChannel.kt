...
object AmarCommandSigner {
    ...
    fun canonical(envelope: AmarCommandEnvelope): String = listOf(
        ...
        envelope.nonce,
        envelope.issuedAtMs.toString(),
        envelope.expiresAtMs.toString(),
        envelope.accountLogin.toString(),
        envelope.botMagic.toString(),
        envelope.symbol,
        envelope.command.requestId,
        envelope.command.symbol,
        envelope.command.side.name,
        decimal(envelope.command.quantity),
        envelope.command.price?.let(::decimal).orEmpty()
    ).joinToString("|")
    ...
    fun verify(envelope: AmarCommandEnvelope, secret: String): Boolean {
        ...
        return MessageDigest.isEqual(
            ...
            envelope.signature.toByteArray(Charsets.US_ASCII)
        )
    }
}
...
enum class AmarCommandRejectReason {
    ...
    EXPIRED, REPLAYED, DUPLICATE, INVALID_SCOPE, INVALID_REQUEST, INVALID_SIGNATURE
}
...
class AmarSecureCommandValidator(private val clockMs: () -> Long, private val maxFutureSkewMs: Long = 30_000L) {
    ...
    fun validate(
        ...
        val now = clockMs()
        if (envelope.issuedAtMs > now + maxFutureSkewMs || envelope.expiresAtMs <= now) {
            return reject(AmarCommandRejectReason.EXPIRED, "انتهت صلاحية الأمر")
        }
        ...
        if (envelope.command.symbol != envelope.symbol) {
            return reject(AmarCommandRejectReason.INVALID_SCOPE, "رمز الأمر لا يطابق رمز الغلاف")
        }
        if (envelope.requestId != envelope.command.requestId) {
            return reject(AmarCommandRejectReason.INVALID_REQUEST, "معرف الأمر غير متطابق")
        }
        // A live-capable command must always be authenticated with receiver-side HMAC.
        if (signatureSecret.isNullOrBlank()) {
            return reject(AmarCommandRejectReason.INVALID_SIGNATURE, "مفتاح التوقيع مطلوب")
        }
        if (!AmarCommandSigner.verify(envelope, signatureSecret)) {
            return reject(AmarCommandRejectReason.INVALID_SIGNATURE, "توقيع الأمر غير صالح")
        }
