        ...
        val errors = buildList {
            ...
            if (entry != null && entry.isFinite() && entry > 0.0) {
                if (intent.side == AmarManualTradeIntent.Side.BUY) {
                    if (intent.stopLoss != null && intent.stopLoss.isFinite() && intent.stopLoss >= entry) {
                        add("STOP_LOSS_SIDE_INVALID")
                    }
                    if (intent.takeProfit != null && intent.takeProfit.isFinite() && intent.takeProfit <= entry) {
                        add("TAKE_PROFIT_SIDE_INVALID")
                    }
                } else {
                    if (intent.stopLoss != null && intent.stopLoss.isFinite() && intent.stopLoss <= entry) {
                        add("STOP_LOSS_SIDE_INVALID")
