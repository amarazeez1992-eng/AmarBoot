...
class MainActivity : ComponentActivity() {
    ...
    private var themeMode = AmarThemeMode.DARK
    ...
    private fun buildHomeWebView(): WebView = WebView(this).apply {
        webViewClient = object : WebViewClient() {
            override fun onPageFinished(view: WebView?, url: String?) {
                runCatching { startBackgroundSystemsOnce() }.onFailure { error ->
                    Log.e("AMAR_STARTUP", "Home page initialization failed", error)
                    showStartupError("تهيئة واجهة AMAR AI", error)
                }
            }
    ...
    private inner class AmarAndroidBridge {
        ...
        fun ask(text: String?) {
            ...
            lifecycleScope.launch {
                ...
                runCatching { agentEngine.ask("", "", request) }
                    .onSuccess { sendAgentResult(it.answer, "Agent: جاهز") }
                    .onFailure { error ->
                        sendAgentResult(
    ...
    private fun startBackgroundSystemsOnce() {
        ...
        onBackPressedDispatcher.addCallback(this, object : androidx.activity.OnBackPressedCallback(true) {
            override fun handleOnBackPressed() {
                home?.evaluateJavascript("window.amarAiGoHome && window.amarAiGoHome()", null)
            }
        })
    }
        ...
        lifecycleScope.launch {
            repeatOnLifecycle(Lifecycle.State.RESUMED) {
                AmarAiAppCommandBus.commands.collect { command ->
                    when (command) {
                        is AmarAiAppCommandBus.Command.OpenRoom -> openNewWorkspaceView(command.room.name)
    ...
    private fun openNewWorkspaceView(roomName: String) {
        val view = when (roomName) {
            "SETTINGS" -> "settings"
            "MARKET" -> "research"
            "ANALYSIS" -> "research"
            "LIBRARY" -> "files"
            "NEWS_SESSIONS" -> "research"
            else -> "home"
        }
        home?.evaluateJavascript("window.amarAiOpenView && window.amarAiOpenView(${org.json.JSONObject.quote(view)})", null)
    }

    private fun onThemeModeChanged(mode: AmarThemeMode) {
        themeMode = mode
        home?.evaluateJavascript("window.amarAiSetTheme && window.amarAiSetTheme(${org.json.JSONObject.quote(mode.name)})", null)
    }

    private fun applyBackground(background: String) {
        home?.evaluateJavascript("window.amarAiSetBackground && window.amarAiSetBackground(${org.json.JSONObject.quote(background)})", null)
    }

    private fun applyAccentColor(color: String) {
        home?.evaluateJavascript("window.amarAiSetAccentColor && window.amarAiSetAccentColor(${org.json.JSONObject.quote(color)})", null)
    }

    private fun requestApplicationUpdate() {
        home?.evaluateJavascript("window.amarAiRequestUpdate && window.amarAiRequestUpdate()", null)
    }

    private fun installProtectionHandler() {
        val previous = Thread.getDefaultUncaughtExceptionHandler()
        Thread.setDefaultUncaughtExceptionHandler { thread, error ->
            runCatching { AmarProtectionCenter.recordFailure(this, "بدء التشغيل", error) }
