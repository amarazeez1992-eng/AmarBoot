...
export default function QuantumWorkspace({ bridge }: Props) {
  ...
  const verifiedResult = response?.verification === 'confirmed' && response?.status === 'complete';
  ...
  useEffect(() => {
    try {
      localStorage.setItem('amar-ui-settings', JSON.stringify({ theme, color: editorColor, size: editorSize, weight: editorWeight, scale: cardScale, notes }));
    } catch { /* UI preferences are non-critical. */ }
  ...
  useEffect(() => {
    ...
    return bridge.onProgress((stage) => setProgressStage(stage));
  }, [bridge]);
  ...
  function applyResponse(result: AmarAgentResponse) {
    ...
    setStatus(result.status ?? (result.verification === 'confirmed' ? 'complete' : 'uncertain'));
  ...
  async function submitPrompt() {
    ...
    setStatus('thinking');
    try {
      const result = await bridge.send({ id: crypto.randomUUID(), prompt, mode: view, searchMode });
      applyResponse(result);
    } catch {
      setStatus('error');
      setResponse({ id: crypto.randomUUID(), text: 'تعذر الحصول على حالة صحيحة من طبقة Item 10.', status: 'error', verification: 'blocked' });
    }
  }
  ...
  async function runResearch() {
    ...
    if (!bridge?.search) {
      setStatus('blocked');
      setResponse({ id: crypto.randomUUID(), text: 'البحث غير متاح بعد في عقد Item 10 المتصل.', status: 'blocked', verification: 'blocked' });
      return;
    }
    setResponse(null);
    setShowEvidence(false);
    ...
    try {
      ...
      setSources(result);
      setProgressStage('verification');
      setStatus(result.length ? 'analyzing' : 'uncertain');
    } catch {
      setStatus('error');
      setResponse({ id: crypto.randomUUID(), text: 'تعذر الحصول على نتيجة بحث موثقة.', status: 'error', verification: 'blocked' });
    }
  }
  ...
  async function analyzeFile(file: File) {
    if (!bridge?.analyzeFile) {
      setStatus('blocked');
      setResponse({ id: crypto.randomUUID(), text: 'تحليل هذا الملف غير متاح بعد في عقد Item 10 المتصل.', status: 'blocked', verification: 'blocked' });
      return;
    }
    setResponse(null);
    setShowEvidence(false);
    ...
    setStatus('analyzing');
    try {
      const result = await bridge.analyzeFile(file);
      applyResponse(result);
    } catch {
      setStatus('error');
      setResponse({ id: crypto.randomUUID(), text: 'تعذر الحصول على حالة تحقق صحيحة لتحليل الملف.', status: 'error', verification: 'blocked' });
    }
  }

  async function toggleScreenShare() {
    if (!screenSharing) {
      if (!bridge?.startScreenShare) {
        setStatus('blocked');
        setResponse({ id: crypto.randomUUID(), text: 'مشاركة الشاشة غير متاحة بعد في عقد Item 10 المتصل.', status: 'blocked', verification: 'blocked' });
        return;
      }
      try {
  ...
  function createNote() {
    ...
    setNotes((items) => [...items, { id, title: `صفحة ${items.length + 1}`, body: '' }]);
    setActiveNoteId(id);
    setNoteDraft('');
  }
  function openNote(note: Note) { setActiveNoteId(note.id); setNoteDraft(note.body); }
  function saveNote() { if (!activeNoteId) return; setNotes((items) => items.map((n) => n.id === activeNoteId ? { ...n, body: noteDraft } : n)); }
  function deleteNote() { if (!activeNoteId) return; setNotes((items) => items.filter((n) => n.id !== activeNoteId)); setActiveNoteId(null); setNoteDraft(''); }
  async function copyText(text: string) { try { await navigator.clipboard.writeText(text); } catch { /* browser permission may block clipboard. */ } }

  const currentStageIndex = progressStage ? safeStages.findIndex((s) => s.id === progressStage) : -1;
  const evidenceAvailable = Boolean(response?.sources?.length || sources.length) && Boolean(bridge?.capabilities?.evidence);
  const editorStyle = { color: editorColor, fontSize: `${editorSize}px`, fontWeight: editorWeight };

  return (
    <main className={`quantum-app theme-${theme}`} dir="rtl" style={{ '--card-scale': `${cardScale / 100}` } as React.CSSProperties}>
      <div className="q-ambient ambient-a" /><div className="q-ambient ambient-b" /><div className="q-grid" />
      <header className="q-topbar glass">
        <div className="q-brand"><button className="q-logo" onClick={() => setView('home')}><span>Q</span><i /></button><div><strong>AMAR AI</strong><small>ITEM 10 UI · AGENT BOUNDARY</small></div></div>
        <div className="q-top-center"><span className={`status-dot ${meta.tone}`} /><b>{meta.label}</b><em>·</em><span>Presentation / Interaction Layer</span></div>
      ...
      <div className="q-shell">
        <aside className={`q-sidebar glass ${mobileNav ? 'mobile-open' : ''}`}>
          <div className="q-sidebar-head"><span>AMAR AI</span><button onClick={() => setMobileNav(false)}><X size={15} /></button></div>
          ...
          <div className="q-sidebar-foot"><ShieldCheck size={14} /><span>لا تنفيذ من طبقة الواجهة</span></div>
        </aside>
        ...
        <section className="q-main" style={{ transform: `scale(${1 + (cardScale - 100) / 1000})`, transformOrigin: 'top center' }}>
          <div className="q-heading glass">
            <div><span className="eyebrow"><ActiveIcon size={13} /> AMAR AI / {view.toUpperCase()}</span><h1>{view === 'home' ? 'مساحة الذكاء الآمنة' : active.label}</h1><p>واجهة متحركة ومتجاوبة؛ Item 10 يحتفظ بالقرار، التحقق، الأدلة، الذاكرة والحوكمة.</p></div>
          ...
          <section className="search-mode glass">
            <div><small>SEARCH POLICY</small><b>وضع البحث</b><span>اختيار الواجهة فقط؛ لا يتجاوز الصلاحيات أو الموافقات.</span></div>
            <div className="mode-switch">
              <button className={searchMode === 'restricted' ? 'selected' : ''} onClick={() => setSearchMode('restricted')}><LockKeyhole size={15} /> بحث بقيود</button>
              <button className={searchMode === 'open' ? 'selected' : ''} onClick={() => setSearchMode('open')} disabled={capabilities.searchOpen === false}><Globe2 size={15} /> بحث مفتوح</button>
            </div>
          </section>
          ...
          {view === 'research' && <ResearchView sources={sources} onRun={runResearch} enabled={Boolean(bridge?.search)} showEvidence={showEvidence} setShowEvidence={setShowEvidence} evidenceAvailable={evidenceAvailable} />}
          ...
          {view === 'settings' && <SettingsView theme={theme} setTheme={setTheme} editorColor={editorColor} setEditorColor={setEditorColor} editorSize={editorSize} setEditorSize={setEditorSize} editorWeight={editorWeight} setEditorWeight={setEditorWeight} showPalette={showPalette} setShowPalette={setShowPalette} cardScale={cardScale} setCardScale={setCardScale} />}

          {panels.progress && <ProgressPanel status={status} currentStageIndex={currentStageIndex} isRunning={isRunning} onToggle={() => togglePanel('progress')} />}
          ...
          {panels.composer && <section className="composer glass">
            <div className="composer-left">
              <button onClick={() => fileRef.current?.click()} title="ملف"><Paperclip size={17} /></button>
              <button onClick={() => mediaRef.current?.click()} disabled={capabilities.image === false} title={capabilities.image === false ? 'غير متاح بعد' : 'صورة'}><ImageIcon size={17} /></button>
              <button onClick={() => videoRef.current?.click()} disabled={capabilities.video === false} title={capabilities.video === false ? 'غير متاح بعد' : 'فيديو'}><Video size={17} /></button>
              <button onClick={() => setRecording(!recording)} disabled={!capabilities.text} className={recording ? 'recording' : ''} title="صوت">{recording ? <Square size={15} /> : <Mic size={17} />}</button>
              <button onClick={toggleScreenShare} disabled={capabilities.screenShare === false} className={screenSharing ? 'screen-live' : ''} title={capabilities.screenShare === false ? 'غير متاح بعد' : screenSharing ? 'Stop Screen Share' : 'Start Screen Share'}>{screenSharing ? <Square size={15} /> : <MonitorUp size={17} />}</button>
            </div>
            <textarea value={message} onChange={(e) => setMessage(e.target.value)} onKeyDown={(e) => { if (e.key === 'Enter' && !e.shiftKey) { e.preventDefault(); void submitPrompt(); } }} placeholder="اكتب طلبًا… لا تظهر النتيجة قبل حالة تحقق صحيحة من Item 10" rows={2} />
            <button className="send" onClick={() => void submitPrompt()} disabled={!bridge?.send || isRunning}><Send size={16} /></button>
            <PanelToggle visible onClick={() => togglePanel('composer')} />
          </section>}

          {screenSharing && <div className="screen-share-banner glass"><span className="live-dot" /> <b>الشاشة قيد المشاركة</b><span>لا تحكم حساس دون تأكيد المستخدم.</span><button onClick={toggleScreenShare}><Square size={13} /> Stop</button></div>}
          ...
          {panels.editor && <EditorPanel style={editorStyle} showPalette={showPalette} setShowPalette={setShowPalette} onCopy={() => void copyText(noteDraft)} onPaste={async () => { try { const text = await navigator.clipboard.readText(); setNoteDraft((v) => `${v}${text}`); } catch { /* permission denied */ } }} onToggle={() => togglePanel('editor')} />}
...
function HomeView({ onNavigate, capabilities }: { onNavigate: (v: ViewId) => void; capabilities: Props['bridge']['capabilities'] }) {
  const cards: [ViewId, typeof Bot, string, string][] = [
    ['assistant', Bot, 'AI Assistant', 'نص، حالات تحقق، أدلة عند توفرها.'],
    ['research', Globe2, 'Research', 'بحث بقيود أو مفتوح وفق صلاحيات Item 10.'],
    ['workspace', FolderOpen, 'Workspace', 'مفكرة وصفحات متعددة ونسخ محلية للواجهة.'],
    ['files', FileSearch, 'File Analysis', 'ملفات وصور وفيديو دون تنفيذ تلقائي.'],
    ['memory', BrainCircuit, 'Memory', 'واجهة عرض فقط؛ الذاكرة الفعلية خارج UI.'],
    ['analytics', BarChart3, 'Analytics', 'مؤشرات تُعرض فقط عندما تأتي من طبقة موثوقة.'],
  ];
  return <div className="home-grid">{cards.map(([id, Icon, title, text]) => <button className="module-card glass" key={id} onClick={() => onNavigate(id)}><div className="module-icon"><Icon size={19} /></div><div><small>AMAR MODULE</small><b>{title}</b><p>{text}</p></div><ChevronLeft size={16} /></button>)}</div>;
}
