...
def read_store():
    ...
    try:
        ...
        if not isinstance(snapshot, str) or value.get("snapshotHash") != digest(snapshot):
            raise ValueError("snapshot integrity failure")
        return {"revision": int(value.get("revision", 0)), "snapshot": snapshot, "snapshotHash": value["snapshotHash"]}
    except Exception:
...
def valid_snapshot(snapshot):
    try:
        ...
        return all(
            isinstance(bot, dict)
            and isinstance(bot.get("botNumber"), int)
            and bot["botNumber"] >= 1
            and isinstance(bot.get("strategies", []), list)
            for bot in value
        )
    except Exception:
        return False
...
class Handler(BaseHTTPRequestHandler):
    ...
    def do_POST(self):
        if self.path != "/sync": return response(self, 404, {"ok": False, "message": "not found"})
        ...
        try:
            ...
            if length <= 0 or length > 2_000_000: return response(self, 413, {"ok": False, "message": "invalid payload size"})
            payload = json.loads(self.rfile.read(length).decode("utf-8"))
            snapshot = json.dumps(payload.get("snapshot"), ensure_ascii=False, separators=(",", ":"))
            if not valid_snapshot(snapshot): return response(self, 400, {"ok": False, "message": "invalid snapshot"})
            if payload.get("snapshotHash") != digest(snapshot): return response(self, 400, {"ok": False, "message": "snapshot hash mismatch"})
            client_revision = int(payload.get("revision", 0))
            base_hash = str(payload.get("baseHash", ""))
            with LOCK:
                store = read_store()
                if store["revision"] == 0:
                    write_store(1, snapshot); store = read_store()
                elif client_revision != store["revision"] or base_hash != store["snapshotHash"]:
                    return response(self, 409, {"ok": False, "message": "sync conflict", "revision": store["revision"], "snapshot": json.loads(store["snapshot"]), "snapshotHash": store["snapshotHash"]})
                else:
                    if digest(snapshot) != store["snapshotHash"]:
                        write_store(store["revision"] + 1, snapshot)
                    store = read_store()
            return response(self, 200, {"ok": True, "revision": store["revision"], "snapshot": json.loads(store["snapshot"]), "snapshotHash": store["snapshotHash"]})
...
if __name__ == "__main__":
    if not TOKEN: raise SystemExit("AMAR_SYNC_TOKEN is required")
